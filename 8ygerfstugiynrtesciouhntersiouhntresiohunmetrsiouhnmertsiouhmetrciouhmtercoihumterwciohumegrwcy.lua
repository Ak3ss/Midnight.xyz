...
settings = Library:addTab("settings", "")
    ...
    local OtherEsp = Tabs.visuals:createGroup('right', 'Other Esp')
    ...
    G2L["4"]["TextColor3"] = Color3.fromRGB(0, 255, 255);
    ...
    CrosshairVertical.Color = Color3.fromRGB(0, 255, 255)
    CrosshairVertical.Thickness = 1
    CrosshairVertical.Visible = false
    local CrosshairHorizontal = Drawing.new("Line")
    CrosshairHorizontal.Color = Color3.fromRGB(0, 255, 255)
    CrosshairHorizontal.Thickness = 1
    CrosshairHorizontal.Visible = false

    OtherEsp:addToggle({text = "Crosshair",default = false,flag = "Crosshair2",callback = function(state)
        CrosshairVertical.Visible = state
        CrosshairHorizontal.Visible = state
    end}) 

    OtherEsp:addToggle({text = "Inventory Viewer",default = false,flag = "InvToggle",callback = function(state)
        G2L["1"]["Enabled"] = state;
    end}) 
    ...
    ExploitsTab:addToggle({text = "No Recoil",default = false,flag = "Special4",callback = function(state)
        setconstant(debug.getproto(trident.gc.recoil, 1), 2, state and trident.gc.classes.InputManager.InputDevice or "Mobile")
    end})
    ExploitsTab:addToggle({text = "No Spread",default = false,flag = "Special5",callback = function(state)
        nospread = state
    end})
    local top = workspace.Const.Ignore.LocalCharacter.Top
    local originalprismcframe = top.Prism1.CFrame
    ExploitsTab:addToggle({text = "Long Neck",default = false,flag = "Special7",callback = function(state)
        top.Prism1.CFrame = state and originalprismcframe - Vector3.yAxis * 5 or originalprismcframe
    end}):addKeybind({text = "Long Neck Bind",type = "toggle",key = Enum.KeyCode.X,flag = "longneckkeybind_toggle",callback = function(state2)
        top.Prism1.CFrame = state2 and originalprismcframe - Vector3.yAxis * 5 or originalprismcframe
    end})
    --
    Nova = Nova or {}
        ...
        function char:UpdateESP(PlayerTable)
            ...
            Connection = RunService.Heartbeat:Connect(function()
                if plr and plr:FindFirstChild("HumanoidRootPart") then
                    ...
                    local Weapon = PlayerTable.equippedItem and PlayerTable.equippedItem.type or "None"
                    --
